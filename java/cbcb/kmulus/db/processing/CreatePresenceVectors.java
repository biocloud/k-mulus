 ...
 */
public class CreatePresenceVectors extends Configured implements Tool {
	
		private static final Logger LOG = Logger.getLogger(CreatePresenceVectors.class);

		private static final String USAGE = "CreatePresenceVectors SEQUENCE_INPUT OUTPUT [NUM_TASKS]";
		...
		public static class Map extends Mapper<LongWritable, Text, LongWritable, PresenceVector> {
			...
			public void map(LongWritable key, Text value, Context context) 
				...
				for (int i = 0; i <= sequence.length() - kmerLength; i += kmerLength)
					featureVector.setKmer(Biology.getAAHash(sequence.substring(i, i + kmerLength)));
	...
	public static void main(String[] args) {
		...
		try {
			result = ToolRunner.run(new CreatePresenceVectors(), args);
	...
	public int run(String[] args) throws Exception {
		...
		LOG.info("Tool name: CreatePresenceVectors");
		LOG.info(" - sequenceInputDir: " + sequenceInputPath);
		LOG.info(" - outputDir: " + outputPath);

		Job job = new Job(getConf(), "CreatePresenceVectors");
		job.setJarByClass(CreatePresenceVectors.class);
		...
		job.setMapperClass(CreatePresenceVectors.Map.class);
		// job.setReducerClass(CreatePresenceVectors.Reducer.class);
